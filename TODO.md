...
- [x] 提取 price anchor（现价/波动）供估值与叙事调用，并将最新价/市值持久化以便离线估值。
  - 后续增强：
    ...
    - [x] 调整估值假设（WACC/g/multiple 分位）入模板和 CLI 参数，暴露敏感性（含情景表 base/bull/bear）。
    ...
    - [x] 行业先验：申万行业映射 + 成分 `daily_basic` 估值分位、beta→CAPM WACC 落入 `valuation_hints` 并模板透明展示。
    - [x] 弱输入提示：EPS/股本缺失等写入 `valuation_warnings`，报告展示。
...
- [ ] 提升「公司概览」「行业与竞争格局」的细节密度：叙事 prompt 强化业务/地区/渠道/KPI/同业对比，QA 提升长度门槛，但仍需更多真实样例验证信息量。
- [x] 规范新闻/定性输出为分条 bullet（日期/来源/情绪/要点），避免长串句子；确保脚注仅文末（已做 bullet 化与占位清洗，继续观察输出质量）。
- [x] 在模板中突出情景估值（base/bull/bear）和行业分位的阅读性（表格化：HTML 与 Markdown 均已完成）。
- [x] 异常提示摘要化：增长异常列表按指标汇总区间，避免长串噪声。
- [x] 图表扩展：新增 Margin Bridge（最新期 waterfall），已有价格/营收净利/利润率/现金流/杠杆流动性。
